...
new mp_friendlyfire, cvar_only_flash_enemy
...
public plugin_init() 
{ 
	register_plugin("ep1c: Flash Amiga", VERSION, "Numb / ConnorMcLeod | Wilian M.") 
	
	RegisterHam(Ham_Think, "grenade", "xHam_ThinkGrenade") 
	
	register_forward(FM_FindEntityInSphere, "xFM_FindEntityInSphere") 
	
	mp_friendlyfire = get_cvar_pointer("mp_friendlyfire") 
	cvar_only_flash_enemy = register_cvar("nft_only_flash_enemy", "0")
	
	g_iMaxPlayers = get_maxplayers() 
} 

public xHam_ThinkGrenade( iEnt ) 
{ 
	...
	if(flDmgTime <= flGameTime && get_pdata_int(iEnt, 114) == 0 // has a bit when is HE or SMOKE 
...
public xFM_FindEntityInSphere(iStartEnt, Float:fVecOrigin[3], Float:flRadius) 
{ 
	const Float:FLASHBANG_SEARCH_RADIUS = 1500.0

	if(flRadius == FLASHBANG_SEARCH_RADIUS && get_gametime() == g_flCurrentGameTime) 
	{ 
		...
		{ 
			...
			{ 
				...
				{ 
					...
					if(get_pcvar_num(cvar_only_flash_enemy))
					{
						if(flFraction == 1.0 && (friendlyfire || cs_get_user_team_index(id) != cs_get_user_team_index(g_iCurrentFlasher) ))
						{ 
					...
					{
						if(flFraction == 1.0 && (friendlyfire || id == g_iCurrentFlasher || cs_get_user_team_index(id) != cs_get_user_team_index(g_iCurrentFlasher) ))
						{ 
							forward_return(FMV_CELL, id)
							return FMRES_SUPERCEDE
						} 
