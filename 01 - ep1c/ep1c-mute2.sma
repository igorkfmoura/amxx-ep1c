...
public show_menu_mute(id)
{
    ...
    formatex(g_item, charsmax(g_item), "Microfone: %s.", g_voiceoff[id][0]?"\r(Desativado)":"\y(Ativado)")
    menu_additem(menu, g_item, "2", 0, g_callback)
    
    menu_additem(menu, "Visualizar quem mutou você.", "3", 0, g_callback)
    
    formatex(g_item, charsmax(g_item), "Alltalk: %s.^n", (g_voiceoff[id][1] || !get_pcvar_num(cvar_alltalk))?"\r(Desativado)":"\y(Ativado)")
...
public show_menu_muteaplayer(id)
{
    ...
    for(new i = 1; i <= g_maxplayers; i++)
    {
        ...
        if(g_mute[id][i])
        {
            formatex(g_item, charsmax(g_item), "\r%s", name)
