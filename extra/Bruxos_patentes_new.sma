...
#define PLUGIN "CS Bruxos: Patents"
#define VERSION "1.0"
...
#define PREFIXCHAT "!gABSOLUTTT;*"
#define PREFIXMENUS "\y[\rABSOLUTTT;*\y]"
...
public plugin_init()
{
	...
	xRegisterSay("ativar", "clcmd_MyRank")
...
public clcmd_MyRank( id )
{
	new xMyRank = xMyPosRankSave[id];

	new szMenu[ 500 ];
	formatex( szMenu, 499, "\r((\dABSOLUTTT\r)) \y~ \wTOP10 \r(VIP)\w:" );

	new h_Menu = menu_create( szMenu, "handler_MyRank" );

	if( xMyRank <= 10 )
		menu_additem( h_Menu, "Ativar VIP", "1" );
	else
		menu_additem( h_Menu, "\dAtivar VIP^n» Entre no TOP10", "1" );

	menu_display( id, h_Menu );
	return PLUGIN_HANDLED;
}

public handler_MyRank( id, menu, item )
{
	new xMyRank = xMyPosRankSave[id];
	...
	switch( item )
	{
		case 0:
		{
			...
			if( equal( authid, "VALVE_4:4", 9 ) )
			{
				set_dhudmessage(random(256),random(256),random(256), 0.01, 0.6, 2, 0.0, 8.0, 0.04, 0.2);
				show_dhudmessage(id, "%s - Ative seu sXe para ganhar VIP!",name);
			}
			else if( xMyRank <= 10 )
			{
				new flags = read_flags("is");
				set_user_flags(id,flags);

				set_dhudmessage(random(256),random(256),random(256), -1.0, 0.6, 2, 0.0, 8.0, 0.04, 0.2);
				show_dhudmessage( 0, "Jogador: %s.^nGanhou VIP. (Rank: %d)",name,xMyRank);
			}
			else
			{
				set_dhudmessage(random(256),random(256),random(256), 0.01, 0.6, 2, 0.0, 8.0, 0.04, 0.2);
				show_dhudmessage(id, "%s - Entre no Top10! Rank: %d",name,xMyRank);	
			}
		}
	}
	return PLUGIN_HANDLED;
...
public xTaskTopsEntry(id)
{
	...
	if(equal(xGetAuth, "", 18))
	{
		xClientPrintColor(0, "%s !tJogador !y%s !ttambém conhecido como !g[!yEDITOR FODÃO!g] !tentrou no servidor.", PREFIXCHAT, xPName)
		client_cmd(0, "speak buttons/blip1")
	}
}
...
public xShowMsgWelcome(id)
{
	...
	set_dhudmessage(random(256),random(256),random(256), 0.06, 0.33, 2, 0.0, 8.0, 0.02, 0.2)
...
public xMotdHelp(id)
{
	...
	<style>body{background: #000 url(^"https://i.imgur.com/hwH8aVF.jpg^") no-repeat fixed center;}table, th, td{border: 1px solid black;border-collapse: collapse;}</style></head><body><table width=100%% cellpadding=2 cellspacing=0 border=1><tr align=center bgcolor=#eeeeee><th width=110%%>AJUDA</tr>")
...
public xViewPatentPlayerMotd(id)
{
	...
	<style>body{background: #000 url(^"https://i.imgur.com/hwH8aVF.jpg^") no-repeat fixed center;}table, th, td{border: 1px solid black;border-collapse: collapse;}</style></head>\
...
public xRankMotd(id)
{
	...
	new iLen
	iLen = formatex(xMotd, charsmax(xMotd), "<html><head><meta charset=UTF-8>\
	<style>body{background: #000 url(^"https://i.imgur.com/hwH8aVF.jpg^") fixed center;}table, th, td{border: 1px solid black;border-collapse: collapse;}</style></head><body><table width=100%% cellpadding=2 cellspacing=0 border=1><tr align=center bgcolor=#eeeeee><th width=110%%>SEU RANK!</tr>")
	
	iLen += formatex(xMotd[iLen], charsmax(xMotd) - iLen, "<tr style=^"color:#fff;font-size:130%%^">")
	iLen += formatex(xMotd[iLen], charsmax(xMotd) - iLen, "<td><br><br>Rank: %s / %s<br>Kills: %s<br>Mortes: %s<br><br>Patente: %s / Prox.: %s<br>Level: %d<br>Experiência: %s / %s",xAddPoint(xMyPosTop10), xAddPoint(xNtvGetTotalTop10()), xAddPoint(xPlayerKills[id]), xAddPoint(xPlayerDeaths[id]), xPatents[xPlayerLevel[id]][xRankName], xPatents[xPlayerLevel[id]+1][xRankName], xPlayerLevel[id], xAddPoint(xPlayerXP[id]), xAddPoint(xPatents[xPlayerLevel[id]+1][xRankXp]))
			...
			case 1:
			{
				if(xPlayerLevel[xKiller] < MAXLEVEL_CSGO-1)
				{
					if(xPlayerXP[xKiller] >= xPatents[xPlayerLevel[xKiller]+1][xRankXp])
					{
						...
						client_cmd(0, "speak ambience/lv_fruit1")
...
public xCreateMotdTop10()
{
	...
	switch(xRandomCss)
	{
		case 0:
		{
			iLen = formatex(xMotd, charsmax(xMotd), "<meta charset=UTF-8>\
			<style>*{margin:0px;}body{color:#fff;background:url(^"http://goo.gl/RBEw1K^")}table{border-collapse:collapse;border: 1px solid #000;text-align:center;}</style>\
			<body><table width=100%% height=100%% border=1><tr bgcolor=#4c4c4c style=^"color:#fff;^"><th width=5%%>#<th width=50%%>NOME<th width=15%%>KILLS\
			<th width=15%%>MORTES<th width=10%%>XP")
		}

		case 1:
		{
			iLen = formatex(xMotd, charsmax(xMotd), "<meta charset=UTF-8>\
			<style>*{margin:0px;}body{color:#fff;background:url(^"https://i.imgur.com/hwH8aVF.jpg^")}table{border-collapse:collapse;border: 1px solid #000;text-align:center;}</style>\
			<body><table width=100%% height=100%% border=1><tr bgcolor=#4c4c4c style=^"color:#fff;^"><th width=5%%>#<th width=50%%>NOME<th width=15%%>KILLS\
			<th width=15%%>MORTES<th width=10%%>XP")
		}
	}
	...
	for(j = 0; j < iSize; j++)
	{
		...
		switch(get_pcvar_num(xCvarPttRankStyle))
		{
			case 1:
			{
				...
				if(j < xMaxColors)
				{
					iLen += formatex(xMotd[iLen], charsmax(xMotd) - iLen, "<tr style=color:%s><td><img src=^"%s^"/> %i<td>%s<td>%s<td>%s<td>%s", xTopColors[j], xTopTrophy[j], j + 1, szName, xAddPoint(str_to_num(szPlayerKills)), 
					xAddPoint(str_to_num(szPlayerDeahts)), xAddPoint(xPlayerXpRank), xGetUserImgRank(xPlayerLvlRank))
				}
				else
				{
					iLen += formatex(xMotd[iLen], charsmax(xMotd) - iLen, "<tr><td>%i<td>%s<td>%s<td>%s<td>%s", j + 1, szName, xAddPoint(str_to_num(szPlayerKills)),
					xAddPoint(str_to_num(szPlayerDeahts)), xAddPoint(xPlayerXpRank), xGetUserImgRank(xPlayerLvlRank))
				}
...
public xHudInfo(id)
{
	...
	switch(get_pcvar_num(xCvarPttRankStyle))
	{
		case 1:
		{
			...
			{
				set_hudmessage(0, 191, 255, 0.02, 0.20, 2, 0.01, 1.0, 0.01, 1.0)

				if(xPlayerLevel[id] < MAXLEVEL_CSGO-1)
				{
					if(equali(xPatents[xPlayerLevel[id]][xRankName], xPatents[xPlayerLevel[id]+1][xRankName]))
						ShowSyncHudMsg(id, xMsgSync[0], "• Patente: %s^n• Prox. Patente: Suba mais seu level.^n• Level: %d^n• Exp: %s / %s^n• Rank: %d", xPatents[xPlayerLevel[id]][xRankName], xPlayerLevel[id], xAddPoint(xPlayerXP[id]), xAddPoint(xPatents[xPlayerLevel[id]+1][xRankXp]), xMyPosRankSave[id])
					else
						ShowSyncHudMsg(id, xMsgSync[0], "• Patente: %s^n• Prox. Patente: %s^n• Level: %d^n• Exp: %s / %s^n• Rank: %d", xPatents[xPlayerLevel[id]][xRankName], xPatents[xPlayerLevel[id]+1][xRankName], xPlayerLevel[id], xAddPoint(xPlayerXP[id]), xAddPoint(xPatents[xPlayerLevel[id]+1][xRankXp]), xMyPosRankSave[id])
				}
				else
				{
					ShowSyncHudMsg(id, xMsgSync[0], "• Patente: %s^n• Level: %d^n• Exp: %s^n• Rank: %d", xPatents[xPlayerLevel[id]][xRankName], xPlayerLevel[id], xAddPoint(xPlayerXP[id]), xMyPosRankSave[id])
				}
			}
			else if(xPlayerHudInfo[id])
			{
				...
				set_hudmessage(47, 79, 79, 0.02, 0.20, 2, 0.01, 1.0, 0.01, 1.0)

				if(!xPlayerHudGeoIp[id2] || equal(xUserCity, "") || equal(xUserRegion, ""))
					ShowSyncHudMsg(id, xMsgSync[0], "Observando: %s^n^n• Patente: %s^n• Level: %d^n• Exp: %s^n• Rank: %d", xPlayerName, xPatents[xPlayerLevel[id2]][xRankName], xPlayerLevel[id2], xAddPoint(xPlayerXP[id2]), xMyPosRankSave[id2])
				else ShowSyncHudMsg(id, xMsgSync[0], "Observando: %s^n^n• Patente: %s^n• Level: %d^n• Exp: %s^n• Rank: %d^n• Cidade: %s^n• Estado: %s", xPlayerName, xPatents[xPlayerLevel[id2]][xRankName], xPlayerLevel[id2], xAddPoint(xPlayerXP[id2]), xMyPosRankSave[id2], xUserCity, xUserRegion)
			}
		}
		...
		case 2:
		{
			if(is_user_alive(id) && xPlayerHudInfo[id])
			{
				...
				if(xPlayerLevel[id] < MAXLEVEL_CSGO2-1)
				{
					ShowSyncHudMsg(id, xMsgSync[0], "• Patente: %s^n• Prox. Patente: %s^n• Level: %d^n• Exp: %s / %s^n• Rank: %d", xPatents2[xPlayerLevel[id]][xRankName], xPatents2[xPlayerLevel[id]+1][xRankName], xPlayerLevel[id],
					xAddPoint(xPlayerXP[id]), xAddPoint(xPatents2[xPlayerLevel[id]+1][xRankXp]), xMyPosRankSave[id])
				}
				else
				{
					ShowSyncHudMsg(id, xMsgSync[0], "• Patente: %s^n• Level: %d^n• Exp: %s^n• Rank: %d", xPatents2[xPlayerLevel[id]][xRankName], xPlayerLevel[id], xAddPoint(xPlayerXP[id]), xMyPosRankSave[id])
				}
			}
			else if(xPlayerHudInfo[id])
			{
				...
				if(!xPlayerHudGeoIp[id2] || equal(xUserCity, "") || equal(xUserRegion, ""))
					ShowSyncHudMsg(id, xMsgSync[0], "Observando: %s^n^n• Patente: %s^n• Level: %d^n• Exp: %s^n• Rank: %d", xPlayerName, xPatents2[xPlayerLevel[id2]][xRankName], xPlayerLevel[id2], xAddPoint(xPlayerXP[id2]), xMyPosRankSave[id2])
				else ShowSyncHudMsg(id, xMsgSync[0], "Observando: %s^n^n• Patente: %s^n• Level: %d^n• Exp: %s^n• Rank: %d^n• Cidade: %s^n• Estado: %s", xPlayerName, xPatents2[xPlayerLevel[id2]][xRankName], xPlayerLevel[id2], xAddPoint(xPlayerXP[id2]), xMyPosRankSave[id2], xUserCity, xUserRegion)
			}
		}
