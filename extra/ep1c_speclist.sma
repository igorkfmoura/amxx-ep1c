...
public cmdSpecList(id)
{
	if( gOnOff[id] )
	{
		client_print(id, print_chat, "[AMXX] Voc� n�o ver� quem est� telando voc�.");
		gOnOff[id] = false;
	}
	else
	{
		client_print(id, print_chat, "[AMXX] Voc� ver� quem est� telando voc�.");
		gOnOff[id] = true;
	}
...
public tskShowSpec()
{
	...
	for( new alive = 1; alive <= gMaxPlayers; alive++ )
	{
		...
		format(szHud, 45, "Telando o bot  %s:^n", szName);
