...
#define PLUGIN_NAME "CS Revo: Dropar Granadas"
...
public plugin_init() {
	register_plugin(PLUGIN_NAME, PLUGIN_VERSION, "VEN | Wilian M.") // register plugin
...
public clcmd_drop(id) {
	...
	if(fWeaponStay > 0)
	{
		remove_task(nade + TASK_NADE)
		set_task(fWeaponStay, "weapon_startFade", nade + TASK_NADE)
	}
...
public weapon_startFade(ent)
{
	...
	weapon_fadeOut(ent + TASK_NADE, 255.0)
}
public weapon_fadeOut(ent, Float:fStart)
{
	...
	if(fFadeAmount[ent] > 0.0)
	{
		...
		set_task(0.1, "weapon_fadeOut", ent + TASK_NADE)
	}
	else
	{
		remove_entity(ent)
