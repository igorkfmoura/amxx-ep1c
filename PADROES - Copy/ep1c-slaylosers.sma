...
public plugin_init()
{
	register_plugin("ep1c: Slay Losers", "1.1", "d3n14@yahoo.com")
...
public slay_those_losers(parm[], effects)
{
	...
	for(new i = 0; i < playersnum; ++i)
	{
		...
		show_hudmessage(id, "Covarde!!^nFalhar não é uma opção!^nMorra!");

		static name[32];
		get_user_name(id, name, charsmax(name));
		client_print_color(0, id, "%s O bot do ^3%s^1 não cumpriu objetivo.", PREFIX, name);
