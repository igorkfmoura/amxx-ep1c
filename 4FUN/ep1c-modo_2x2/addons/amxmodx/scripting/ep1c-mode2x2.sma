...
#define ID_KEY_WALL			54345678
					...
					//x	//y
#define MESSAGE_MAP_STATUS		-1.0,	0.8				//  Posição da mensagem sobre o Fechamento / Abertura do cartão.


														//r	//green		//blue
#define COLOR_MAP_CLOSE			255,	0,		0		// A cor da mensagem quando online é baixa e o cartão está fechado. Tipo de cor RGB, http://www.colorschemer.com/online.html
#define COLOR_MAP_OPEN			0,	255,		0		// A cor da mensagem quando online é mais alta do que o necessário e o cartão é aberto. Tipo de cor RGB, http://www.colorschemer.com/online.html

#define MODE_TIME_START			10.0	// Quanto tempo demora para começar a votar, após o número de votos necessário.
#define MODE_COUNT_START		5	// Relatório pré-votação
#define VOTE_TIMEWAIT			3	// Quantos minutos depois de votar /mode, estará disponível novamente.
#define VOTE_RATIO			0.5	// Precisão para o número de votos, Exemplo: (Ratio: 0.5, obrigatório 0.5 * 32 = 16 votos de 32 jogadores)

#define STRONG_PUSH			15.0	// Força de empurrar a caixa de arma (arma, C4) da parede
#define MAX_CLIENTS			32

#define PREFIX				"\1[\4[ep1c gaming Brasil]\1]"
#define CLASSNAME_WALL			"test_effect" // unused classname in the game
#define SPRITE_WALL			"sprites/ep1c/98992.spr"
...
public plugin_init()
{
	register_plugin("ep1c: Mode 2x2", PLUGIN_VERSION, "s1lent");
	...
	pcvar_mode_admin_only = register_cvar("mode_admin_only", "0");				// Evita que o segundo administrador use o comando "/change", se o primeiro administrador já o usou para abrir o cartão e se o administrador que ativou "/change" - ativo e no comando.
	pcvar_mode_allow_vote = register_cvar("mode_allow_vote", "1");				// Permitir usar o comando "/mode"
	pcvar_mode_allow_change = register_cvar("mode_allow_change", "1");			// Permitir usar o comando "/change"
	pcvar_mode_force_check_online = register_cvar("mode_force_check_online", "1");		// Force a verificação online, mesmo quando /change, se não houver administradores ativos no jogo, exceto para administradores no espectador, se mode_admin_only, verifica apenas um administrador ativo.
	pcvar_mode_block_startgame = register_cvar("mode_block_startgame", "0");		// Nunca coloque paredes quando "GameCommencing" ou "Restart"
	pcvar_mode_push_weaponbox = register_cvar("mode_push_weaponbox", "1");			// Empurrar weaponbox (braços, C4) da parede

	pcvar_mode_message_on_touch = register_cvar("mode_messagetouch", "0");			// Informe o jogador ao tocar na parede que a passagem está fechada.
	pcvar_mode_touch_wait = register_cvar("mode_messagetouch_time", "5.5");			// Atraso para re-mensagem quando o jogador toca a parede.
	pcvar_mode_changemapname = register_cvar("mode_changemapname", "1");

	Load_Config();

	register_clcmd("say /mode", "CMD_Mode", 0, "<Votação para abrir / fechar um cartão>");
	register_clcmd("say /change", "CMD_ModeChange", ADMIN_VOTE, "<Mudança de modo Modo 2x2, Abrir / fechar mapa>");

	register_clcmd("say /box", "CMD_MenuBox", ADMIN_RCON, "<Gerenciamento de objetos, criar / modificar / excluir>");
	register_dictionary("ep1c-mode.txt");
...
stock CreateBox(bool:bParse = false)
{
	...
	if (bParse)
	{
		...
		// entity_set_float(pEnt, EV_FL_renderamt, 230.0);
