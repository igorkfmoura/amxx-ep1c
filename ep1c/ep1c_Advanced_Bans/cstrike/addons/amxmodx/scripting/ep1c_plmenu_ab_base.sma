...
public cmdBanReason(id)
{
	...
	log_amx("Ban: ^"%s<%d><%s><>^" ban and kick ^"%s<%d><%s><>^" (minutes ^"%d^") (reason ^"%s^")", name, get_user_userid(id), authid, name2, userid2, authid2, banTime, reason);
	...
	{
		client_cmd(id, "amx_banip #%i %i ^"%s^"", userid2, banTime, reason);
		if (g_tempBans)
		{
			new ipa[32];
	...
	{
		client_cmd(id, "amx_ban #%i %i ^"%s^"", userid2, banTime, reason);
		if (g_tempBans)
		{
			new ipa[32];
			get_user_ip(player, ipa, charsmax(ipa), 1);
...
public actionBanMenu(id, key)
{
	switch (key)
	{
		...
		default:
		{
			...
			g_banPlayer[id] = g_menuPlayers[id][g_menuPosition[id] * 7 + key];

			new name2[MAX_NAME_LENGTH];
			get_user_name(g_banPlayer[id], name2, charsmax(name2));
			...
			if (banTime)
			{
				formatex(tmp, charsmax(tmp), "por ^3%d^1 minutos", banTime);
			}
			else
			{
				formatex(tmp, charsmax(tmp), "de forma ^3PERMANENTE^1");
			}
			client_print_color(id, print_team_red, "^3[ep1c gaming Brasil]^1 Digite o ^4motivo^1 para banir ^4%s^1 %s.", name2, tmp);
		}
	}
