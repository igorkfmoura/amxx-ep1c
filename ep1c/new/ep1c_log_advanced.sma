...
public client_say(id) {
	...
	if((cvar_value == 1 || cvar_value == 3) && (flags == 0 || (player_flags & flags) || separate)) {
		...
		formatex(main_holder[len], charsmax(main_holder) - len, "^n%s, %s, %s, say, %s, ^"%s^", ^"%s^"", 
		cur_date, cur_hours, team, is_user_alive(id) ? "alive" : "dead", name, args)
...
public client_say_team(id) {
	...
	if((cvar_value == 1 || cvar_value == 3) && (flags == 0 || (player_flags & flags) || separate)) {
		...
		formatex(main_holder[len], charsmax(main_holder) - len, "^n%s, %s, %s, say_team, %s, ^"%s^", ^"%s^"", 
		cur_date, cur_hours, team, is_user_alive(id) ? "alive" : "dead", name, args)
...
print_details(id, log, console, chat, flagged_player, separate, type_data) {
	...
	new comma
	if(log) { 
		formatex(log_data_string_start[len], charsmax(log_data_string_start) - len, "^n%s, %s, ", cur_date, cur_hours)
	}
	if(cvar_data & (1<<0)) {
	...
	len += formatex(log_data_string[len], charsmax(log_data_string) - len, ", ^"%s^"", map_name)
	len += formatex(log_data_string[len], charsmax(log_data_string) - len, ", %s", type_data ? "disconnected" : "connected")
