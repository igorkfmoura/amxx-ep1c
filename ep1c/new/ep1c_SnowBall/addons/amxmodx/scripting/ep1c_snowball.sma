...
public plugin_init()
{
	...
	register_touch("grenade", "*", "FwdTouch")

	register_cvar("amx_sbgo", "1")
	SB_GIVEOUT = get_cvar_num("amx_sbgo")
	register_cvar("amx_deffect", "1")
	DEATH_EFFECT = get_cvar_num("amx_deffect")
	register_cvar("amx_sbtrail", "1")
	TRAIL = get_cvar_num("amx_sbtrail")
	register_cvar("amx_sbcolor", "1")
	TRAIL_COLOR = get_cvar_num("amx_sbcolor")
	SBCVAR = register_cvar("amx_sbdmg", "50")
	SBDMG = get_pcvar_float(SBCVAR)
	
	register_message(get_user_msgid("TextMsg"), "block_message")
	register_message(get_user_msgid("SendAudio"), "block_audio")
	register_message(get_user_msgid("WeapPickup"), "block_message_wpu")
...
{
	register_cvar("amx_sbmode", "1")	
	SBMODE = get_cvar_num("amx_sbmode")
...
{
	if(is_user_connected(victim) && DEATH_EFFECT)
	{
	static pOrigin[3]
...
public fw_EmitSound(id, channel, const sample[], Float:Volume, Float:attn, flags, pitch)
{
	...
	{
	if(SBMODE && equal(sample, sounds[i])) return FMRES_SUPERCEDE
	}
	return FMRES_IGNORED
}
...
{
	if(SBMODE) return PLUGIN_HANDLED
	return PLUGIN_CONTINUE
}

public block_message(msg_id, msg_dest, entity)
{
	if(get_msg_args() == 5 && SBMODE)
	{
