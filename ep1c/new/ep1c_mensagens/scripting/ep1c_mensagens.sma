...
new const PLUGIN[] = "ep1c: Mensagens";
new const VERSION[] = "0.6";
new const AUTHOR[] = "MaximusBrood + lonewolf";
...
public plugin_init() {
	...
	register_clcmd("ad_reload", "load", ADMIN_CVAR);
...
public load(id)
{
	...
	format(filepath, 63, "%s/ep1c-mensagens.ini", filepath);
	
	server_print("[%s] loading '%s'", PLUGIN, filepath);
	if (is_user_connected(id))
	{
		console_print(id, "Reloading ads '%s'", filepath);
	}
	...
	if(file_exists(filepath))
	{
		...
		if(adCount[NORM_AD] != 0 && !task_exists(72202))
			set_task(random_float(RAND_MIN, RAND_MAX), "eventTask", 72202);
