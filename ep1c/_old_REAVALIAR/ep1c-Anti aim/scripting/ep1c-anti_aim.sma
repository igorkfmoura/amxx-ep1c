...
#define ENABLE_LOG_FILE					// Habilitar registro
#define RELOAD_CMD 		ADMIN_CFG		// Sinalizador de acesso para o comando config reload: reaim_reloadcfg
#define MENU_CMD 			ADMIN_BAN		// Sinalizador de acesso ao menu de objetivo: say /aim
...
public plugin_init()
{
	register_plugin("ep1c: Anti-Aim", REAIMDETECTOR_VERSION, "ReHLDS Team");
...
public AimMenu(id, level)
{
	...
	if(bFindPlayer)
	{
		...
		menu_setprop(iMenu, MPROP_EXITNAME, "Resultado");
		menu_setprop(iMenu, MPROP_NEXTNAME, "Avançar");
		menu_setprop(iMenu, MPROP_BACKNAME, "Retroceder");
	...
	{
		new szBufNotify[190];
		formatex(szBufNotify, charsmax(szBufNotify), "^1[^4Aim Detector^1] ^3Nenhum ^1player ^4detectado ^3atualmente^1.");
