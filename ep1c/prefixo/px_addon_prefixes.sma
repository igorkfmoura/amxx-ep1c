...
public plugin_init()
{
	register_plugin("\r[PX] \y[ADDON]: \wChat Manager", PLUGIN_VERSION, "BhK-")
...
public show_expire(id)
{
	...
	if(g_expired[id] == -1)
		client_print_color(id, print_team_default, "^4[^1PX^4] ^1Parabéns seu admin é ^4Permanente^1!!!")

	else
		client_print_color(id, print_team_default, "^4[^1PX^4] ^1Seu admin termina no dia: ^4%s ^3| Estado atual: ^4%s", g_szDate[id], g_expired[id] ? "VENCIDO" : "ATIVADO")
